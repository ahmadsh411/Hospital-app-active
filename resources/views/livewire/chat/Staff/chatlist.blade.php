    ...
    <div class="card shadow-sm border-0">
        <div class="main-content-left main-content-left-chat">
            ...
            <nav class="nav main-nav-line main-nav-line-chat border-bottom">
                <a class="nav-link active text-dark" href="#">{{ __('messages.recent_conversations') }}</a>
            </nav>

            <!-- قائمة جهات الاتصال -->
            <div class="main-chat-contacts-wrapper p-3">
                <!-- قائمة جهات الاتصال بميزة التمرير -->
                <ul class="list-group" id="chatmodel" style="max-height: 400px; overflow-y: auto;" wire:poll.1s="loadConversations">
                    @foreach($conversations as $conversation)
                        @php
                            // البريد الإلكتروني للطرف الآخر في المحادثة
                            $currentEmail = auth('staff')->user()->email;
                        ...
                        <li class="list-group-item d-flex align-items-center border-0 py-3">
                            ...
                            <div class="d-flex flex-column">
                                <button wire:click="userSelectedstaff('{{$contactEmail}}')" style="border: none;background-color: transparent">
                                    ...
                                    <small class="text-muted">
                                        ...
                                        {{ $conversation->messages()->latest()->first() ? $conversation->messages()->latest()->first()->body : __('messages.no_messages_yet') }}
                                    </small>
                                    <small class="text-muted">
                                        {{ $conversation->messages()->latest()->first() ? $conversation->messages()->latest()->first()->created_at->diffForHumans() : __('messages.no_messages_yet') }}
                                    </small>
                                </button>
                            </div>
                        </li>
            ...
            @if($conversations->isEmpty())
                <div class="main-chat-list text-center p-3" id="ChatList">
                    <p class="text-muted">{{ __('messages.no_conversations_to_display') }}</p>
                </div>
            @endif
        </div>
    </div>
