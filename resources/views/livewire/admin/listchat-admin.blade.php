    ...
    <div class="card shadow-sm border-0">
        <div class="main-content-left main-content-left-chat">
            ...
            <nav class="nav main-nav-line main-nav-line-chat border-bottom">
                <a class="nav-link active text-dark" href="#">{{ __('messages.Recent Conversations') }}</a>
            </nav>
                ...
                <ul class="list-group" id="chatmodel" style="max-height: 400px; overflow-y: auto;" wire:poll.1s="loadConversations">
                    @foreach($conversations as $conversation)
                        @php
                            ...
                            $currentEmail = auth('admin')->user()->email;
                            ...
                            <div class="d-flex flex-column">
                                <button wire:click="userSelected('{{ $contactEmail }}')" style="border: none; background-color: transparent">
                                    <h6 class="mb-0 font-weight-bold text-dark">
                                        {{ \App\Models\Conversations\Conversation::getUserNameByEmail($contactEmail) }}
                                    </h6>
                                    <small class="text-muted">{{ $contactEmail }}</small>
                                    <small class="text-muted">
                                        {{ __('messages.Last Message') }}:
                                        {{ $conversation->messages()->latest()->first() ? $conversation->messages()->latest()->first()->body : __('messages.No Messages Yet') }}
                                    </small>
                                    <small class="text-muted">
                                        {{ $conversation->messages()->latest()->first() ? $conversation->messages()->latest()->first()->created_at->diffForHumans() : __('messages.No Messages Yet') }}
                                    </small>
                                </button>
                            </div>
                        </li>
            ...
            @if($conversations->isEmpty())
                <div class="main-chat-list text-center p-3" id="ChatList">
                    <p class="text-muted">{{ __('messages.No Conversations Available') }}</p>
                </div>
            @endif
        </div>
    </div>
