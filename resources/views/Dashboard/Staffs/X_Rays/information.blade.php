    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('messages.ray_info') }}</span>
            </div>
        </div>
    </div>
@endsection
...
@section('content')
    ...
    <div class="container mt-5">
        <h2 class="text-primary">{{ __('messages.ray_details_for') }}: {{ $ray->patient->name }}</h2>

        <div class="card shadow-lg mb-4">
            <div class="card-body">
                <h5 class="text-info">{{ __('messages.ray_info') }}</h5>
                <p><strong>{{ __('messages.patient_name') }}:</strong> {{ $ray->patient->name }}</p>
                <p><strong>{{ __('messages.description') }}:</strong> {{ $ray->description ?? __('messages.no_description') }}</p>
                <p><strong>{{ __('messages.status') }}:</strong> {{ $ray->status ? __('messages.completed') : __('messages.not_completed') }}</p>
                <p><strong>{{ __('messages.exam_date') }}:</strong> {{ $ray->staff_date }}</p>
            </div>
        </div>
        ...
        <div class="card shadow-lg mb-4">
            <div class="card-body">
                ...
                <div class="d-flex flex-wrap">
                    @php $sendImages = $ray->images->where('type', 0); @endphp
                    @if($sendImages->count() > 0)
                        @foreach($sendImages as $index => $image)
                            <img
                                src="{{ asset('Dashboard/img/x-rays/'.$ray->patient->name.'/send/'.$ray->id.'/'.$image->filename) }}"
        ...
        <div class="card shadow-lg mb-4">
            <div class="card-body">
                ...
                <div class="d-flex flex-wrap">
                    @php $readyImages = $ray->images->where('type', 1); @endphp
                    @foreach($readyImages as $index => $image)
                        <img
                            src="{{ asset('Dashboard/img/x-rays/'.$ray->patient->name.'/Ready/'.$ray->staff->name.'/'.$ray->id.'/'.$image->filename) }}"
                            class="img-thumbnail m-2"
                            style="width: 150px; height: 150px; cursor: pointer;"
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                <p><strong>{{ __('messages.staff_name') }}:</strong> {{ $ray->staff_name ?? __('messages.not_available') }}</p>
                <p><strong>{{ __('messages.staff_description') }}:</strong> {{ $ray->staff_description ?? __('messages.not_available') }}</p>
            </div>
        </div>
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                <p><strong>{{ __('messages.doctor') }}:</strong> {{ $ray->doctor->name ?? __('messages.not_available') }}</p>
            </div>
        </div>
    </div>
@endsection
        ...
        let images = {
            'send': [
                @foreach($sendImages as $image)
                    "{{ asset('Dashboard/img/x-rays/'.$ray->patient->name.'/send/'.$ray->id.'/'.$image->filename) }}",
                @endforeach
            ],
            'ready': [
                @foreach($readyImages as $image)
                    "{{ asset('Dashboard/img/x-rays/'.$ray->patient->name.'/Ready/'.$ray->staff->name.'/'.$ray->id.'/'.$image->filename) }}",
                @endforeach
            ]
        };
