<!-- Modal لإضافة أشعة -->
<div class="modal fade" id="addlaboratoryModal{{ $invoice->id }}" tabindex="-1" role="dialog" aria-labelledby="addRayLabel{{ $invoice->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="addRayLabel{{ $invoice->id }}">
                    <i class="fas fa-x-ray"></i> {{ __('messages.add_laboratory_for', ['name' => $invoice->patient->name]) }}
                </h5>
                ...
                <form action="{{ route('laboratories.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="description">
                            <i class="fas fa-file-alt"></i> {{ __('messages.laboratory_description') }}
                        </label>
                    ...
                    <div class="form-group">
                        <label for="image">
                            <i class="fas fa-upload"></i> {{ __('messages.upload_image') }}
                        </label>
                        <input type="file" class="form-control" id="image" name="image" accept="image/*" required onchange="previewImage(event)">
                    </div>
                    <!-- مكان عرض الصورة -->
                    <div class="form-group text-center">
                        <img id="imagePreview1" src="" alt="{{ __('messages.image_preview') }}" style="max-width: 100%; height: auto; display: none; border: 2px solid #ddd; padding: 5px; margin-top: 10px;">
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save"></i> {{ __('messages.save_laboratory') }}
                        </button>
                    </div>
                </form>
            </div>
